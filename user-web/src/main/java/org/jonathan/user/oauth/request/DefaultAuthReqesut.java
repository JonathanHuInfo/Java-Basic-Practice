...
import org.jonathan.user.oauth.config.AuthSource;
...
import org.jonathan.user.utils.UuidUtils;
...
public abstract class DefaultAuthReqesut implements AuthReqeust {
    ...
    public AuthResponse login(AuthCallback authCallback) {
        try {
            ...
            return new AuthResponse().setCode(200).setData(authUser);
        } catch (Exception e) {
    ...
    private AuthResponse errorResponse(Exception e) {
        ...
        return  new AuthResponse().setCode(errorCode).setMsg(errorMsg);
    }
